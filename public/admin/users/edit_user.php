...
<?php
if (is_post_request()) {
//    $allowed = $_SESSION['priv_level'] > 1 && $user['priv'] <= $_SESSION['priv_level'];
    $user['id'] = $id;
    ...
    $result = edit_user($user, ["password_required" => $new_password,
        "email_is_already_taken" => $email_is_already_taken
    ]);

    if ($result === true) {
        redirect_to(url('admin/users/index.php?id=' . $id));
    ...
    <form class="form" action="<?php echo url('/admin/users/edit_user.php?id=' . h(u($id))); ?>" method="post"
        ...
        <h1 class="formTitle">Edit User</h2>
            ...
            <p type="Privilege Level:">
                <select name="priv">
                    <?php
                    $max_priv = $_SESSION['priv_level'] > 1 ? $_SESSION['priv_level'] - 1 : $_SESSION['priv_level'];
            ...
            <p type="Visible:">
                ...
                <input type="checkbox" name="active" value="y" <?php if ($user['active'] == 'y') {
                    echo " checked";
} ?>/>
            </p>
